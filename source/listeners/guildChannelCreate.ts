...
export class GuildChannelCreate extends Listener {
	...
	public async run(channel: GuildChannel) {
		...
		const embed = new EmbedBuilder()
			.setColor('#802e2e')
			.setAuthor({ name: 'Guild Channel Create', iconURL: this.container.client.user?.displayAvatarURL() })
			.addFields([
			...
			.setFooter({ text: 'Logging-Module: channelCreate' });
